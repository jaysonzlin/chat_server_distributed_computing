# This holds the previous implementation of the GUI, for local comparison.
...
class UnreadInboxPage(tk.Frame):
    ...
    Page #5:
      ...
      - Has a number input (default=5). Calls load_unread_messages(n).
      - Each message has a "Read Message" button => calls read_message(msg_id).
        On success, refresh the inbox.
      - Contains a button to go to the ReadMessagesPage.
      - Contains a button to go to the Send Message page.
    """
    def __init__(self, parent, controller: "App"):
        super().__init__(parent)
        self.controller = controller
        ...
        # Controls at bottom: number input, Refresh, Send Message, and Go to Read Messages
        ...
        goto_read_btn = tk.Button(
            control_frame, text="Go to Read Messages", command=self.goto_read_messages
        )
    ...
    def on_refresh(self):
        """Reload with the number from self.num_var."""
        try:
...
class ReadMessagesPage(tk.Frame):
    ...
    Page #6:
      ...
      - Has a number input to load_read_messages(n).
      - "Delete Selected" deletes selected messages.
      - Contains a button to go to the Unread Inbox.
      - Contains a button to go to the Send Message page.
    """
    def __init__(self, parent, controller: "App"):
        super().__init__(parent)
        ...
        delete_btn = tk.Button(control_frame, text="Delete Selected", command=self.on_delete_selected)
        delete_btn.pack(side="left", padx=5)

        send_message_btn = tk.Button(control_frame, text="Send Message", command=self.goto_send_message)
        send_message_btn.pack(side="left", padx=5)

        goto_unread_btn = tk.Button(
            control_frame, text="Go to Unread Inbox", command=self.goto_unread_inbox
        )
...
class SendMessagePage(tk.Frame):
    """
    New Page:
      - Contains a dropdown input to select the recipient (excluding the current user).
      - Contains a multi-line text input field to type a message.
      - At the bottom, a Send button calls the send_message function using the logged-in username.
      - Displays the result of success or error returned from the server.
      - Also has a button to go to the Read Messages page.
    """
    def __init__(self, parent, controller: "App"):
        super().__init__(parent)
        self.controller = controller
    ...
    def display_result(self, result_message):
        # Display the server response result in the label.
...
class App(tk.Tk):
    """
    Main Tk app controlling 7 pages:
      1) UsernamePage
      2) LoginPage
      ...
      5) UnreadInboxPage
      6) ReadMessagesPage
      7) SendMessagePage  <-- New page for sending messages
    """
